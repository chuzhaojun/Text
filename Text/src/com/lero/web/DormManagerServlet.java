...
public class DormManagerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			con = dbUtil.getCon();
			List<DormManager> dormManagerList = dormManagerDao.dormManagerList(
					con, pageBean, dormManager);
	...
	private void dormManagerDelete(HttpServletRequest request,
		...
		try {
			...
			request.getRequestDispatcher("dormManager?action=list").forward(
					request, response);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void dormManagerSave(HttpServletRequest request,
		...
		DormManager dormManager = new DormManager(userName, password, name,
				sex, tel);
		...
		try {
			...
			} else if (dormManagerDao.haveManagerByUser(con,
			...
			if (saveNum > 0) {
				request.getRequestDispatcher("dormManager?action=list")
						.forward(request, response);
			} else {
				request.setAttribute("dormManager", dormManager);
				request.setAttribute("error", "����ʧ��");
				request.setAttribute("mainPage",
						"dormManager/dormManagerSave.jsp");
				request.getRequestDispatcher("mainAdmin.jsp").forward(request,
						response);
			}
		} catch (Exception e) {
	...
	private void dormManagerPreSave(HttpServletRequest request,
		...
		if (StringUtil.isNotEmpty(dormManagerId)) {
			...
			try {
				con = dbUtil.getCon();
				DormManager dormManager = dormManagerDao.dormManagerShow(con,
						dormManagerId);
				request.setAttribute("dormManager", dormManager);
			} catch (Exception e) {
				e.printStackTrace();
	...
	private String genPagation(int totalNum, int currentPage, int pageSize) {
		...
		if (currentPage == 1) {
			...
			pageCode.append("<li><a href='dormManager?page="
					+ (currentPage - 1) + "'>��һҳ</a></li>");
		}
		for (int i = currentPage - 2; i <= currentPage + 2; i++) {
			...
			if (i == currentPage) {
				...
				pageCode.append("<li><a href='dormManager?page=" + i + "'>" + i
						+ "</a></li>");
			}
		}
		...
		pageCode.append("<li><a href='dormManager?page=" + totalPage
				+ "'>βҳ</a></li>");
